# Capabilities — web_application + task_tracker
...
- `GET|POST /projects`, `GET|PUT|DELETE /projects/{id}` — Project CRUD (auth + owner-scoped)
- `GET|POST /tasks`, `GET|PUT|DELETE /tasks/{id}` — Task CRUD (auth + owner-scoped)
- `GET|POST /labels`, `GET|PUT|DELETE /labels/{id}` — Label CRUD (auth required (shared across users))
