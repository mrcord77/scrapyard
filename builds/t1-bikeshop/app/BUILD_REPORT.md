# Build report — Bicycle Repair Shop

Domain `bikeshop` · data sensitivity **low**.
...
- Security: auth, write-role=admin
- Indexed (owner/external, no FK): `user_id`
- Workflow: yes

## Workflows

### RepairTicket.status (initial: `intake`)
- `intake` → `diagnosed`
- `diagnosed` → `ready`
- `ready` → `picked_up`
  - entering `ready` requires `parts_received` == `True` — else: cannot mark ready: parts not received
  - entering `picked_up` requires `paid` == `True` — else: cannot pick up: invoice unpaid
