# Capabilities — saas_subscription_app + saas
...
- `GET|POST /accounts`, `GET|PUT|DELETE /accounts/{id}` — Account CRUD (PUBLIC — no auth (low-sensitivity tier))
- `GET|POST /members`, `GET|PUT|DELETE /members/{id}` — Member CRUD (PUBLIC — no auth (low-sensitivity tier))
- `GET|POST /plans`, `GET|PUT|DELETE /plans/{id}` — Plan CRUD (PUBLIC — no auth (low-sensitivity tier))
- `GET|POST /invitations`, `GET|PUT|DELETE /invitations/{id}` — Invitation CRUD (PUBLIC — no auth (low-sensitivity tier))
