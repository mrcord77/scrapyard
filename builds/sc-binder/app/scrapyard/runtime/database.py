...
def _ensure_security_tables(engine, security_caps, create: bool):
    ...
    if "account_deletion" in (security_caps or []):
        # the erasure route records a DeletionRecord (compliance evidence);
        # without this table the hard-delete path fails at commit time.
        from scrapyard.compliance.account_deletion import DeletionRecord
        wanted.append(DeletionRecord.__table__)
    if create:
        for t in wanted:
