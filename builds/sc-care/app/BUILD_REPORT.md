# Build report — Care Circle

Domain `care_circle` · data sensitivity **moderate**.
...
### CareTask  (`care_tasks`)
- Security: auth
- Indexed (owner/external, no FK): `recipient_id`
- Workflow: yes
- Reference rule: `recipient_id` must reference an existing `CareRecipient`
...
- Workflow: no
- Reference rule: `medication_id` must reference an existing `Medication` with status in ['active', 'paused']
...
### CareTask.status (initial: `open`)
- `open` → `claimed`, `dropped`
- `claimed` → `done`, `open`, `missed`
- `missed` → `open`, `escalated`
- `escalated` → `done`
- `done` → 
- `dropped` → 

### Medication.status (initial: `active`)
- `active` → `paused`, `discontinued`
- `paused` → `active`, `discontinued`
- `discontinued` → 

### Appointment.status (initial: `scheduled`)
