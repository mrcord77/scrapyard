# Capabilities — basic_saas + care_circle
...
- `GET|POST /care_recipients`, `GET|PUT|DELETE /care_recipients/{id}` — CareRecipient CRUD (auth required (shared across users))
- `GET|POST /care_tasks`, `GET|PUT|DELETE /care_tasks/{id}` — CareTask CRUD (auth required (shared across users))
- `GET|POST /medications`, `GET|PUT|DELETE /medications/{id}` — Medication CRUD (auth required (shared across users))
- `GET|POST /dose_logs`, `GET|PUT|DELETE /dose_logs/{id}` — DoseLog CRUD (auth required (shared across users))
- `GET|POST /appointments`, `GET|PUT|DELETE /appointments/{id}` — Appointment CRUD (auth required (shared across users))
- `GET|POST /updates`, `GET|PUT|DELETE /updates/{id}` — Update CRUD (auth required (shared across users))
