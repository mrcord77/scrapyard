# Capabilities — ticketing_system + construction
...
- `GET|POST /projects`, `GET|PUT|DELETE /projects/{id}` — Project CRUD (PUBLIC — no auth (low-sensitivity tier))
- `GET|POST /tasks`, `GET|PUT|DELETE /tasks/{id}` — Task CRUD (PUBLIC — no auth (low-sensitivity tier))
- `GET|POST /change_orders`, `GET|PUT|DELETE /change_orders/{id}` — ChangeOrder CRUD (PUBLIC — no auth (low-sensitivity tier))
- `GET|POST /documents`, `GET|PUT|DELETE /documents/{id}` — Document CRUD (PUBLIC — no auth (low-sensitivity tier))
