# Capabilities — agent_platform + research_workbench
...
`account_lockout`, `app_factory`, `auth_routes`, `base_model`, `chunking`, `config`, `cors`, `db_session`, `document_store`, `email`, `email_verification`, `embeddings`, `empty_states`, `error_handling`, `error_taxonomy`, `forms`, `frontend.dashboards`, `health`, `input_sanitization`, `jwt_manager`, `llm_client`, `logging_setup`, `middleware`, `migrations`, `navbars`, `pagination`, `pagination_params`, `password_hashing`, `password_policy`, `password_reset`, `providers`, `rate_limiting`, `repository`, `request_context`, `routers`, `secrets`, `security_headers`, `session_manager`, `settings_pages`, `settings_validation`, `soft_delete`, `tables`, `timestamps`, `transactions`, `users`, `validation`

## Runnable endpoints

- `GET /healthz`, `GET /livez` — health/liveness
- `POST /auth/register`, `POST /auth/login`, `POST /auth/logout`, `GET /auth/me`
- `GET|POST /research_docs`, `GET|PUT|DELETE /research_docs/{id}` — ResearchDoc CRUD (auth + owner-scoped)
- `GET|POST /notes`, `GET|PUT|DELETE /notes/{id}` — Note CRUD (auth + owner-scoped)
- `GET|POST /experiments`, `GET|PUT|DELETE /experiments/{id}` — Experiment CRUD (auth + owner-scoped)
- `GET|POST /runs`, `GET|PUT|DELETE /runs/{id}` — Run CRUD (auth + owner-scoped)
- `GET|POST /tags`, `GET|PUT|DELETE /tags/{id}` — Tag CRUD (auth required (shared across users))
