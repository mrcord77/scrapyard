# Build report — Recovery / sobriety support

Domain `sobriety` · data sensitivity **high**.
...
- Entities requiring auth: **9**
- Entities owner-scoped: **6**
...
### Post  (`posts`)
- Security: auth, owner=user_id, encrypted=body, audit=create,update,delete
- Indexed (owner/external, no FK): `user_id`
- Workflow: no

### Sponsor  (`sponsors`)
- Security: auth, audit=create,delete
- Indexed (owner/external, no FK): `sponsor_user_id`, `sponsee_user_id`
- Workflow: no
...
- Security: auth, owner=user_id, audit=create,delete
- Foreign keys: `meeting_id` → `meetings`
- Indexed (owner/external, no FK): `user_id`
- Workflow: no
...
- Security: auth, owner=user_id, encrypted=body,mood, audit=create,delete
- Indexed (owner/external, no FK): `user_id`
- Workflow: no
