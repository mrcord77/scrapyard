# Capabilities — course_platform + education
...
`account_lockout`, `app_factory`, `auth_routes`, `base_model`, `cancellation_flow`, `config`, `cors`, `db_session`, `email`, `email_verification`, `empty_states`, `entitlement_gate`, `entitlements`, `error_handling`, `error_taxonomy`, `forms`, `frontend.dashboards`, `health`, `input_sanitization`, `invoices`, `jwt_manager`, `logging_setup`, `middleware`, `migrations`, `navbars`, `pagination`, `pagination_params`, `password_hashing`, `password_policy`, `password_reset`, `rate_limiting`, `repository`, `request_context`, `routers`, `sales_tax_filing_calendar`, `sales_tax_nexus`, `secrets`, `security_headers`, `session_manager`, `settings_pages`, `settings_validation`, `soft_delete`, `stripe_checkout`, `stripe_tax`, `stripe_webhooks`, `subscription_status`, `subscriptions`, `tables`, `timestamps`, `transactions`, `users`, `validation`

## Runnable endpoints

- `GET /healthz`, `GET /livez` — health/liveness
- `POST /auth/register`, `POST /auth/login`, `POST /auth/logout`, `GET /auth/me`
- `GET|POST /courses`, `GET|PUT|DELETE /courses/{id}` — Course CRUD (auth required (shared across users))
- `GET|POST /modules`, `GET|PUT|DELETE /modules/{id}` — Module CRUD (auth required (shared across users))
- `GET|POST /lessons`, `GET|PUT|DELETE /lessons/{id}` — Lesson CRUD (auth required (shared across users))
- `GET|POST /enrollments`, `GET|PUT|DELETE /enrollments/{id}` — Enrollment CRUD (auth required (shared across users))
- `GET|POST /progresses`, `GET|PUT|DELETE /progresses/{id}` — Progress CRUD (auth required (shared across users))
