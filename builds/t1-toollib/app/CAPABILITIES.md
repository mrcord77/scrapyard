# Capabilities — basic_saas + tool_library
...
- `GET|POST /tools`, `GET|PUT|DELETE /tools/{id}` — Tool CRUD (PUBLIC — no auth (low-sensitivity tier))
- `GET|POST /reservations`, `GET|PUT|DELETE /reservations/{id}` — Reservation CRUD (PUBLIC — no auth (low-sensitivity tier))
- `GET|POST /incidents`, `GET|PUT|DELETE /incidents/{id}` — Incident CRUD (PUBLIC — no auth (low-sensitivity tier))
- `GET|POST /maintenance_records`, `GET|PUT|DELETE /maintenance_records/{id}` — MaintenanceRecord CRUD (PUBLIC — no auth (low-sensitivity tier))
- `GET|POST /tags`, `GET|PUT|DELETE /tags/{id}` — Tag CRUD (PUBLIC — no auth (low-sensitivity tier))
