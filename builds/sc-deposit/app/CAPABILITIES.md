# Capabilities — basic_saas + deposit_shield
...
- `GET|POST /tenancies`, `GET|PUT|DELETE /tenancies/{id}` — Tenancy CRUD (auth + owner-scoped)
- `GET|POST /evidence_shots`, `GET|PUT|DELETE /evidence_shots/{id}` — EvidenceShot CRUD (auth + owner-scoped)
- `GET|POST /deductions`, `GET|PUT|DELETE /deductions/{id}` — Deduction CRUD (auth + owner-scoped)
- `GET|POST /dispute_letters`, `GET|PUT|DELETE /dispute_letters/{id}` — DisputeLetter CRUD (auth + owner-scoped)
