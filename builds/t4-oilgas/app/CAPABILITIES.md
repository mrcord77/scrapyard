# Capabilities — ticketing_system + oil_and_gas
...
- `GET|POST /wells`, `GET|PUT|DELETE /wells/{id}` — Well CRUD (auth required (shared across users))
- `GET|POST /leases`, `GET|PUT|DELETE /leases/{id}` — Lease CRUD (auth required (shared across users))
- `GET|POST /production_logs`, `GET|PUT|DELETE /production_logs/{id}` — ProductionLog CRUD (auth required (shared across users))
- `GET|POST /work_orders`, `GET|PUT|DELETE /work_orders/{id}` — WorkOrder CRUD (auth required (shared across users))
